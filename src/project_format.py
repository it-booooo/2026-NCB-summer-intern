...
def validate_state(state):
    ...
    if not isinstance(events, list) or len(events) > MAX_EVENTS:
        raise ValueError("Project video marker list is invalid or too large.")
    for event in events:
...
def validate_video_bounds(state, metadata):
    ...
    for event in state.get("events", []):
        if event.get("frame_index", 0) >= metadata.total_frames:
            raise ValueError("A project video marker frame is outside the source video.")
        if float(event.get("video_time_sec", 0.0)) > metadata.duration_sec:
