...
class SignalCsvFixtureTests(unittest.TestCase):
    ...
    def test_dataset_reuses_time_arrays_and_keeps_raw_signal_float32(self):
        config = SignalFixtureConfig(
            sample_rate_hz=10,
            duration_s=1,
            missing_sample_indices=(2,),
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            first_time = dataset.time_us
            first_record_time = dataset.record_time_s
            raw = dataset.signal_values(2)
            segment = dataset.segment(2, 0.0, 0.5, None)

        self.assertIs(dataset.time_us, first_time)
        self.assertIs(dataset.record_time_s, first_record_time)
        self.assertEqual(first_time.dtype, np.dtype("float64"))
        self.assertEqual(first_record_time.dtype, np.dtype("float64"))
        self.assertEqual(raw.dtype, np.dtype("float32"))
